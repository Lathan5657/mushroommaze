...
#include <cmath>
#include <time.h> // time libary for time(NULL)// math libary for rand 
#include <vector>
...
int main ()
{
    ...
    cout << " Welcome to Mushroom Hunter!\n\n";
    cout << "In order to beat this game you must find the Mushroom to give yourself a extra life!!\n";
    cout << " that is located in the maze. You can move using the \n";
    cout << " arrow keys or WASD.\n\n";
    cout << " Warning!!! There are enemies that will take life away and a\n";
    cout << " Mushroom adds a life! Hopefully, you can survive!\n ";
    cout << " Goodluck and have fun!\n\n\n\n";
 
 
	enemies enemy_horizontal = {6, 9, 'X'}; //placing enemy_horizontal at location [9][13] and gives it the sign H
	enemies enemy_vertical = { 3 , 5, 'X'};
	enemies enemy_jump = {1, 7, 'J'};
	mushrooms mushroom_heal = {2, 8, 'M'};

 
 
	const char HEIGHT = 10, WIDTH = 10; // the maze width and height 
	...
	player.hp = 2; // players different attributes 
...
update = true;	// map updates 
	...
	maze[mushroom_heal.posX][mushroom_heal.posY] = mushroom_heal.mushroom_symbol;
 ...
 if ( maze[player.posX ][player.posY ] == maze[mushroom_heal.posX][mushroom_heal.posY])
														...
														// he also returns to his starting point when he can't get any further do to #                                                                           
	{
		...
		(maze[enemy_jump.posX -=2][enemy_jump.posY] != '#')
	{
		maze[enemy_jump.posX][enemy_jump.posY] = ' ';
		enemy_jump.posX -= 2;
	}
	...
	if (player.posY == 9)// when the player reach the bottom line he winsd the game 
	{
